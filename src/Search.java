...
/**
 * Search class implements breadth-first search, depth-first search,
 * uniform-cost search, greedy search and heuristics search in order to solve
 * the sokoban puzzle
 * 
...
public class Search {
	...
	/**
	 * Implementation for breadth-first search. Code is based on textbook pseudocode
	 * 
	 * @param p
	 * @return
	 */
	public String bfs(Problem p) {
		...
		while (!fringe.isEmpty()) {
			...
			// ("+n.state.player.row+","+n.state.player.col+")");
	...
	/**
	 * Implementation for depth-first search. Code is similar to breadth-first
	 * search, but instead of queue, stack is used for implementing LIFO
	 * 
	 * @param p
	 * @return
	 */
	public String dfs(Problem p) {
		...
		return getSolution(DFS, null, totalNode, redundant, fringe.size(), explored.size(),
				System.currentTimeMillis() - startTime);
	}

	/**
	 * Implementation for uniform-cost search, greedy search, and A* search. These
	 * search method uses same code, except for sorting for priorityQueue.
	 * 
	 * @param p
	 * @param choice
	 * @return
	...
	public String prioritySearch(Problem p, char choice) {
		...
		return getSolution(method, null, totalNode, redundant, fringe.size(), explored.size(),
				System.currentTimeMillis() - startTime);
	}

	/**
	 * When solution is found, or when a program fails to solve the puzzle,
	 ...
	 * @return
	 */
	private String getSolution(String method, Node n, int totalNode, int redundant, int fringeSize, int exploredSize,
			long totalTime) {
		String result = "";
		int steps = 0;
		if (n == null)
		...
		// "\nd) Number of nodes on the explored list (if there is one) when termination
		// occurs: " + exploredSize +
	...
	/**
	 * getChild method returns the child node according to the action performed (l,
	 * r, u, d) if isUcs is true, add 1 to newCost whenever box is pushed
	 * 
	 * @param p
	 * @param n
	 * @param action
